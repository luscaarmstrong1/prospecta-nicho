...
test("login admin estático chama admin-login e não expõe service role", async ({ page }) => {
  await page.route("**/functions/v1/admin-login", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ ok: true, session: { accessToken: "jwt-administrativo-de-teste" } }),
    });
  });
  await page.goto("/prospecta-nicho/admin/login/");
  await page.getByLabel(/Email administrativo/i).fill("admin@prospectanicho.test");
  await page.getByLabel(/^Senha$/i).fill("senha-de-teste");

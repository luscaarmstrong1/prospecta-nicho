...
test("solicitação rápida salva antes de exibir WhatsApp", async ({ page }) => {
  ...
  const requestSaved = page.waitForResponse(
    (response) => response.url().includes("/api/custom-requests") && response.request().method() === "POST",
  );
  await page.getByRole("button", { name: /Solicitar esta planilha/i }).click();
  const savedResponse = await requestSaved;

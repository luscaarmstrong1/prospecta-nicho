...
// cspell:ignore Linkedin Youtube testid
...
export function PreviewSiteV2() {
  return (
    <PreviewMotionProvider>
      <PreviewSiteV2Content />
    </PreviewMotionProvider>
  );
}

function PreviewSiteV2Content() {
  ...
  const { motionOff, reducedMotion, pointerFine } = usePreviewMotion();
  ...
  return (
    ...
    >
      ...
      <p className="sr-only">Versão visual de teste. Sem integração com backend. Nenhum dado é enviado.</p>
      <PreviewHeader
        menuOpen={menuOpen}
      ...
      <main id="conteudo-principal">
            ...
            >
              ...
              <div className={styles.buttonRow} data-testid="preview-hero-actions">
                <Magnetic className={styles.magneticWrap}>
                  <button className={styles.primaryButtonLarge} type="button" onClick={() => openMockAction("Montar minha base") }>
        ...
        <section className={styles.sampleSection} id="amostra" aria-labelledby="sample-title" data-testid="preview-sample">
          ...
          <Reveal className={styles.sampleCopy}>
            ...
            <div className={styles.sampleActions}>
              <Magnetic className={styles.magneticWrap}>
                <button className={styles.primaryButtonLarge} type="button" onClick={() => openMockAction("Receber amostra grátis") }>
        ...
        <section className={styles.segmentSection} id="segmentos" aria-labelledby="segments-title" data-testid="preview-segments">
          <Reveal className={styles.sectionHeadingLight}>
            ...
            <button className={styles.linkButtonDark} type="button" onClick={() => openMockAction("Ver todos os segmentos") }>
            ...
            {featuredSegments.map((segment) => (
              <InteractiveCard className={styles.segmentCard} key={segment.title} tilt data-testid="motion-segment-card">
                ...
                <div className={styles.segmentContent}>
                  ...
                  <button type="button" onClick={() => openMockAction(`Acessar ${segment.title}`)}>
          ...
          <div className={styles.planGrid}>
            {plans.map((plan) => (
              <InteractiveCard className={styles.planCard} key={plan.title} data-custom={plan.custom ? "true" : "false"}>
                ...
                <button type="button" onClick={() => openMockAction(plan.custom ? "Falar com um especialista" : `Ver detalhes de ${plan.title}`)}>
          ...
          <Reveal className={styles.finalCtaContent}>
            ...
            <div className={styles.finalCtaAside}>
              ...
              <div className={styles.buttonRow}>
                <Magnetic className={styles.magneticWrap}>
                  <button className={styles.primaryButtonLarge} type="button" onClick={() => openMockAction("Montar meu recorte") }>
                    Montar meu recorte <ArrowRight aria-hidden="true" />
                  </button>
                </Magnetic>
                <button className={styles.secondaryButtonLarge} type="button" onClick={() => openMockAction("Falar com um especialista") }>
        ...
        <Reveal className={styles.footerTop}>
          <div className={styles.footerBrand}>
            ...
            <div className={styles.socials} aria-label="Redes sociais demonstrativas">
              <button type="button" aria-label="LinkedIn" onClick={() => openMockAction("LinkedIn")}><Linkedin aria-hidden="true" /></button>
              <button type="button" aria-label="Instagram" onClick={() => openMockAction("Instagram")}><Instagram aria-hidden="true" /></button>
              <button type="button" aria-label="YouTube" onClick={() => openMockAction("YouTube")}><Youtube aria-hidden="true" /></button>
            </div>
          </div>
          {footerGroups.map((group) => (
            <div className={styles.footerLinks} key={group.title}>
              <h3>{group.title}</h3>
              {group.links.map((link) => <button type="button" key={link} onClick={() => openMockAction(link)}>{link}</button>)}
            </div>
          ))}
      ...
      <AnimatePresence>
        {modalTitle ? <PreviewModal title={modalTitle} onClose={() => setModalTitle(null)} /> : null}
      </AnimatePresence>
    </div>
  );
}

...
import { bodyErrorResponse, enforceRateLimit, readJsonBody } from "../_shared/request-security.ts";
...
Deno.serve(async (request) => {
  ...
  const limited = await enforceRateLimit(request, "admin-login", 5, 300);
  ...
  if (email && password) {
    ...
    if (!url || !anonKey) return errorJson(request, "SUPABASE_AUTH_NOT_CONFIGURED", "Supabase Auth não configurado.", 503);
    const loginResponse = await fetch(`${url.replace(/\/+$/, "")}/auth/v1/token?grant_type=password`, {
      method: "POST",
      headers: { apikey: anonKey, authorization: `Bearer ${anonKey}`, "content-type": "application/json" },
      body: JSON.stringify({ email, password }),
    });
    if (!loginResponse.ok) return errorJson(request, "ADMIN_UNAUTHORIZED", "E-mail ou senha administrativa inválidos.", 401);
    const session = await loginResponse.json() as { access_token?: string; expires_in?: number; user?: { id?: string } };
    const userId = session.user?.id;
    if (!session.access_token || !userId) return errorJson(request, "ADMIN_UNAUTHORIZED", "Sessão administrativa inválida.", 401);
    ...
    if (!allowedRoles.has(role)) return errorJson(request, "ADMIN_FORBIDDEN", "Usuário sem perfil administrativo.", 403);
    return json(request, {
      ok: true,
      session: {
  ...
  if (!expected) return errorJson(request, "ADMIN_TOKEN_NOT_CONFIGURED", "ADMIN_API_TOKEN não configurado no Supabase.", 503);
  if (text(body.token, 200) !== expected) return errorJson(request, "ADMIN_UNAUTHORIZED", "Token administrativo inválido.", 401);
